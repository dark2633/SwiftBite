...
namespace SwiftBite
{
    public partial class frmMcDonalds : Form
    {
        public frmMcDonalds()
        {
            InitializeComponent();
        ...
        private void frmMcDonalds_Load(object sender, EventArgs e)
        {

        }

        private void btnPhilly_Click(object sender, EventArgs e)
        {
            ...
            if (!Globals.McDonaldsitems.Contains(itemKey)) //If the item has not yet been "added to cart"
            {
                Globals.McDonaldsitems.Add(itemKey); //Adds item to cart
                btn.BackgroundImage = Properties.Resources.Add_Item_TICK; //Changes the button's BackgroundImage to display a `✓`
            }

            else
            {
                Globals.McDonaldsitems.Remove(itemKey); //Removes item from cart
                btn.BackgroundImage = Properties.Resources.Add_Item_PLUS; //Changes the button's BackgroundImage to display a `+`
            }

            lblCartNotificationText.Text = Globals.McDonaldsitems.Count.ToString(); //Updates the cart badge to show the number of items "in the cart"
        }
